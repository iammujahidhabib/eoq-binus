    ...
    public function add()
    {
        if (isset($_GET['id'])) {
            ...
            if ($this->input->post()) {
                ...
                $data['PakaiBarang'] = $this->templates->join_where('pemesanan','id_barang','barang', ['id_pesanan' => $this->input->get('id')])->result_array();//where join
                $this->load->view('user/template/header');
                // echo "zzz";
                $this->load->view('user/produksi/pakai_barang/add', $data);
                $this->load->view('user/template/footer');
            }
        } else {
            redirect('produksi/produksi');
